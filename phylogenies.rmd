...
```{r trees-darwin, echo=FALSE, fig.cap="Darwin's depiction of the evolutionary relationships among organisms (Darwin, 1859).", out.width="100%"}
...
```{r trees-haeckel, echo=FALSE, fig.cap="Haeckel's hypothesis of the animal phylogeny (Haeckel, 1897), drawn as an actual tree.", out.width = if (knitr::is_html_output()) "100%"}
# HTML-only out.width; see the note on the trees-graph chunk below. Without it
# this chunk emitted no width attribute at all, so the plate rendered at its
# intrinsic size.
...
```{r trees-graph, echo=FALSE, fig.cap="Simple graph. Nodes are represented by circles, and edges by lines. Note that this graph has a cycle (you could walk in a circle between nodes 1, 2, and 4). It is also not bifurcating.", out.width = if (knitr::is_html_output()) "60%"}
# out.width is set for HTML only. knitr sizes an included raster from its pixel
# dimensions and dpi: at dpi=300 this 749px image is 2.5in, which is right for
# the 4.75in print text block, but the HTML path applies a much smaller factor
# and emits width="120" -- a fraction of the ~770px content column, while the
# R-generated figures fill it. Setting it here fixes HTML and leaves the PDF
# (and therefore its pagination) untouched. 60% rather than 100%: this is a
# six-node schematic, and at full column width it dwarfs the surrounding text.
...
```{r trees-groups, echo=FALSE, fig.cap="(A) A monophyletic group, i.e., a clade. (B) A polyphyletic group. (C) A paraphyletic group that does not include the hare."}
...
```{r trees-rooted-abstract, echo=FALSE, fig.cap="These four cladograms have the same tips and same topology; only the layout differs. (A) The first layout is unrooted. The other layouts are rooted on the black node (B), gray node (C), and gray branch (D). Each of these elements is in the exact same topological position in all trees. When the tree is rooted on the black or gray nodes, the base of the tree is a polytomy since these nodes have three branches attached to them. No nodes are added or removed when rooting on a node. When rooting on the gray branch, a new node is added along the branch. This new node is bifurcating."}
...
p_min = tree_star %>%
  ...
  #geom_tiplab(aes(image=paste0("figures/phylopic/", uid, '.512.png')), geom="image", offset=offset, size=phylopic_size_small ) +
  xlim(0, 2) +
  ggtitle("(A) Unlabeled 11-tip comb cladogram")
...
<!-- Figure 2.13 is tall enough that LaTeX defers it to a float page. Left to
     ...
     has no floats. -->
`r if (knitr::is_latex_output()) '\\FloatBarrier'`
