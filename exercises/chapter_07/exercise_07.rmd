...
We are going to use a third party tool, [FigTree](https://github.com/rambaut/figtree/releases), to view the phylogenies. You may need to also install [java](https://www.oracle.com/java/technologies/javase-jre8-downloads.html) to get FigTree running. 
...
That tutorial is written as if you are running the commands on your own computer. If you prefer to install revayes locally you can do that, or you can run the analyses on the cluster. 

If you run the analyses on the cluster, there are some modifications to make:

- Rather than launch the scripts by opening RevBayes and running a `source()` command, you will use the `job*.sh` slurm scripts to launch revbayes on the cluster and pass the script to it at the command line.
...
To understand the MCMC sampling, plot some key variables recorded in the log file through generations. Here we will plot each replicate (ie chain) as a different color.

The tutorial suggests using a specialized program called Tracer, but we will plot the summary MCMC stats right here in R.
...
> **_Problem 1:_** (1 point) Answer the following questions about the MCMC summaries above.

- Was the burnin sufficient?
- Does mixing look good?
- When considering the whole run, why do you think TL starts high and then gets lower, while Likelihood starts low and gets higher?
- Did the replicates converge on similar parameter distributions?

Open the tree `output_jc/primates_cytb_JC_MAP.tree` on your local computer with FigTree to inspect the tolpology, edge length, and posterior probabilities. You will need to check the `Node Labels` box and then in `Display:` select `posterior`.

> **_Problem 2:_** (1.5 points) Answer the above questions for the GTR analyses. Copy and paste the codeblocks from above and modify them to import these results and plot them. Pick at least one other model paraemter to examine in addition to TL and Likelihood. Note that this analysis has just a single replicate. 

> **_Problem 3:_** (1.5 points) Answer the above questions for the GTR+IG analyses. Copy and paste the codeblocks from above and modify them to import these results and plot them. Examine alpha in addition to TL and likelihood. Note that this analysis has just a single replicate. 
