...
Either on the cluster (which requires copying and modifying a job script) or on your local computer (if you have iqtree installed) run the following:

    iqtree2 -s siph16s.phy -bb 1000 -nt AUTO
...
iqtree2 -s mammal_jc.fasta -nt AUTO
...
iqtree2 -s mammal_hky.fasta -nt AUTO
# GTR - 
...
iqtree2 -s mammal_gtr.fasta -nt AUTO
# GTR IG - 
...
iqtree2 -s mammal_gtr_ig.fasta -nt AUTO
```
...
Confirm that this document knits without error. Compress the exercise folder, with this file, the knitted html file, and all analysis results, and submit.
