...
When you submit your ``.rmd` assignment on Canvas, Gradescope runs all your code and checks the results. You can submit our assignment as many times as you like - the only submission that will count toward your grade is the last one before the deadline. This setup gives you feedback on how you are doing, as often as you want, whenever you want.

You will follow a workflow like this for most exercises:

1. Download the exercise from Canvas. It will be posted before Class on Thursday, and we will usually start working on it in class on Thursday.
...
3. Start editing the file. When you get to a point that you want to check things, re-knit it to confirm that it runs and you get output. If it knits and looks good to you, submit it to test the results. You can work incrementally, checking each new result before heading to the next.

4. Go back to 2 and repeat until all tasks are completed and you are happy with your score.

For this to work, there are a couple things to keep in mind:

- Don't change the name of the file after you download it. The grader expects the exact name of the original file.

- The autograding works by testing the value of some specific variables after running your code. To make it completely clear what is being evaluated, I have prepended `check_` to the names of these graded variables. For example, `check_x`. These will be initialized (often with a `NULL` value) in the file you download, and you will write the code to set their value to something meaningful. Don't change the names of these special variables that will be tested. They must have the right names and right values for you to get full credit.

- The autograders are only as good as the people who write them, in this case me. If you think your result is correct but you aren't getting credit for it, let the me know. You may be doing something consistent with the instructions that I didn't anticipate.

> **_TASK:_** Submit this file on canvas. It should tell you that all your answers are wrong. Congratulations, everything is working!

It is good practice to knit and submit the assignment before you start working. This will let you know everything is running appropriately before you get started, and help pinpoint any bugs that might arise later. If the downloaded assignment won't knit or the autograder tells you there is a problem (other than that you get 0 points), let the professor know so that things can be fixed before you start.

Some future exercises will require the use of tools other than RStudio and the grading will be a bit different. This will be explained when we get to them.
...
After you edit the code, knit this document to see if the results look good. If they do, then submit it on Canvas and check your score on Problem 1.

## Phylogenies in R

This is a phylogenetics course, so let's start by defining and drawing some phylogenies. We'll focus on this [next week](http://dunnlab.org/phylogenetic_biology/phylogenies.html#representation), so this is just a bit of a teaser.

One of the most important tools for phylogenetic analyses in R is the package [ape](https://cran.r-project.org/web/packages/ape/ape.pdf), which is already loaded in this exercise with the `library(ape)` command in the first chunk of this exercise.
...
```{r}
    # add code to create newick text of tree,
    # be sure the tip labels are spelled exactly as above

    # edit the following line to store the phylo representation of the tree in check_phylo_new
    # don't change this variable name, because it starts with check_ you know it is what will be 
    # evaluated when grading the exercise
    check_phylo_new = NULL

    # add code to plot the tree
```
...
> **_Problem 4:_** Edit the code below to specify the label (ie, name) of the species designated for each question. This will require replacing the `NULL` values with species names from the tree above. It will be easier to read the tree figure in the knitted output than to read the newick data above.

```{r}

# Be sure to put quotes around text strings, for example:

x = "dog"

# Which species is sister to Balaenoptera_musculus?
check_sister_balaenoptera = NULL


# Which species is sister to all other species in the tree?
check_sister_all = NULL
