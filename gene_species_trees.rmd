...
gst_draw_tube <- function(main, ab_y, gene="concordant") {
  ...
  if (gene=="concordant") {
    ...
    cBC  <- lin(rootN, topN, 0.34)  # B,C coalesce first, in the ancestral tube
    cABC <- lin(rootN, topN, 0.72)  # then A joins
    gl(tipA, off(abN,oL), off(rootN,oL), cABC)  # A passes through uncoalesced
    gl(tipB, off(abN,oR), off(rootN,oR), cBC)   # B passes through uncoalesced
    gl(tipC, off(rootN,0.12), cBC); dot(cBC)
...
```{r gst-reconciliation, echo=FALSE, fig.width=7, fig.height=3.4, fig.cap="A gene family tree (right) reconciled against a species tree (left). A duplication (D) early in the history produced two copies, and within each copy divergences track speciation, so each copy recapitulates the species tree ((A,B),C). A1 and B1 are orthologs (their common ancestor is a speciation), while A1 and A2 are paralogs (their common ancestor is the duplication). The second copy in species C (C2) was subsequently lost: its lineage terminates before the present (marked with an X). Comparing paralogs by mistake -- say A1 with B2 -- would badly misrepresent how the species are related."}
...
plot(NA, xlim=c(0.4,6.6), ylim=c(4.3,-0.6), axes=FALSE, xlab="", ylab="",
     main="Gene family tree", cex.main=1.1)
text(c(1,2,3,4,5), rep(-0.35,5), c("A1","B1","C1","A2","B2"), font=3, cex=1.0)
# copy 1: ((A1,B1),C1)
segments(1,0,1,1.6); segments(2,0,2,1.6); segments(1,1.6,2,1.6)
segments(1.5,1.6,1.5,2.6); segments(3,0,3,2.6); segments(1.5,2.6,3,2.6)
# copy 2: ((A2,B2),C2), with C2 lost -- its branch stops before the present
segments(4,0,4,1.6); segments(5,0,5,1.6); segments(4,1.6,5,1.6)
segments(4.5,1.6,4.5,2.6); segments(6,1.0,6,2.6); segments(4.5,2.6,6,2.6)
points(6,1.0,pch=4,cex=1.2,lwd=2,col="grey30")
text(6,0.9,"C2\nlost",pos=3,cex=0.72,col="grey30",offset=0.35)
# duplication D joining the two copies
segments(2.25,2.6,2.25,3.4); segments(5.25,2.6,5.25,3.4); segments(2.25,3.4,5.25,3.4)
segments(3.75,3.4,3.75,4.0)
points(3.75,3.4,pch=22,bg="grey40",cex=1.7); text(3.75,3.4,"D",col="white",cex=0.7)
```
