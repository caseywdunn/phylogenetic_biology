...
The process behind incomplete lineage sorting is most naturally described backward in time, in the same spirit as the simulation-first view of models we developed earlier (Chapter \@ref(simulation)). Follow the same gene sampled in three different species back through their ancestral populations. Looking backward, gene lineages **coalesce** when they find a common ancestor. This coalescence will always occur before the species split, the question is how long before. If the coalescence occurs along the branch subtending the speciation, the gene tree will match the species tree (Figure \@ref(fig:gst-coalescent), left); if it occurs deeper in the ancestral population, the gene tree may be discordant (Figure \@ref(fig:gst-coalescent), right).
...
Methods that consider gene duplciation and loss can reduce inference problems aand also let us discard fewer data (since we can relax our gene selection criteria to include genes with more complex histories). Tools that identify gene families and infer their histories, such as OrthoFinder [@emms2019], make the full complement of gene families available for analysis, and methods such as ASTRAL-Pro estimate the species tree directly from multi-copy gene family trees, accounting for paralogy instead of requiring it to be filtered away first [@zhang2020].
...
We have now seen the biological processes that make gene trees depart from the species tree, and the methods that model them. But this machinery is not always needed, and a great deal of phylogenetics proceeds perfectly well while ignoring discordance. Whether that is safe depends on the cause and the biology of the problem at hand. Incomplete lineage sorting can be a serious problem when internal branches are short and populations large, and is of particular concern in recent rapid divergences. Gene duplication and loss matters most when gene families are large and dynamic, and when the greatest time has elapsed along the branches of the species tree. It is therefore of particular concern when estimating very deep phylogenetic problems. Reticulation matters when hybridization or horizontal transfer is common in the group at hand. This propensity varies widely across the tree of life, and is especially common in plants and prokaryotes. In each case, the biology of the group and structure of the phylogenetic problem should inform the choice of method.

Many analyses do not attempt to model discordance at all, and simply treat all regions as though they share one history. This can be a reasonable approach when discordance is expected to be mild, and the extra cost of discordant-aware methods is high. One of the most common multigene analysis approaches is to concatenate all the alignments end to end into one supermatrix and infer a single tree. Concatenation-based approaches and methods that model gene-tree species-tree discordance often, though of course not always, give similar results.
