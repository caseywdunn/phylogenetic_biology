...
```{r sim-tree-cnid, fig.cap="A published phylogeny (Zapata, 2015) with a scale bar indicating branch length in terms of the expected amount of evolutionary change, rather than absolute time.", out.width="100%"}
# out.width pins the figure to the text block; without it knitr sizes an
# included raster from its own pixels and dpi, which can overflow the margin.
...
```{r sim-analytical, fig.cap="The probability of observing a particular end state at time $t$, given the start state A and $\\mu=0.05$. The solid line is the probability of observing the original start state (as described by Equation \\@ref(eq:sim-stay)); the dashed line is the probability of observing each of the three other states (as described by Equation \\@ref(eq:sim-change))." }
...
p + 
  stat_function(fun = fun.1, linetype = "solid") + 
