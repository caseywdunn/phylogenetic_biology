...
There is considerable variation in how the exact same phylogeny can be drawn (Figure \@ref(fig:trees-layouts)). This gives some flexibility in deciding what presentation is best for your particular goals, but can also create confusion because two images that look very different may in fact be of the exact same phylogeny. Rectangular layouts are the most common, because the entire edge length is along one axis of the plot. Slanted layouts avoid the elbows of rectangular layouts, but because the edges aren't parallel and can be at a variety of angles, it isn't always easy to read edge lengths on them. Circular layouts place the root at the center, and the tips around the edges. This is a very compact representation that works well for very large phylogenies. 
...
So far we have dealt with rooted phylogenies. Each of them has had a root node that is the oldest node in the phylogeny. It has no edge connecting it to a parent node, and all other nodes in the phylogeny are its descendants. This seems simple enough.
