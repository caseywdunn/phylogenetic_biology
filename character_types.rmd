...
Since the practice of measurement in evolutionary biology proceeded pragmatically and largely independent of measurement theory, there are some differences in the nomenclature. What phylogenetic biologists call "character type" is referred to in measurement theory, and many other fields of science, as "scale type" (Table \@ref(tab:char-scale-types)). Scale types vary in several ways. The Domain indicates the possible values. Phylogenetic methods differ most based on whether this domain is discrete or continuous, reflected here in the Measurement type column. 

Permissible transformations indicate the mathematical operations that can be performed without distorting measurement meaning. Consider first the nominal scale type. An analyses of DNA would give the exact same results if we used four different symbols for the nucleotides, *i.e.*, a one-to-one mapping from ACGT to XWYZ. This is a mathematical transformation. But there are other transformations that are not permissible in the sense that they provide no meaningful output. For example, addition. A+G doesn't equal another nucleotide. Such a mathematical manipulation of the symbols as no corresponding real world manipulation of the actual nucleotides. Now consider  

Arbitrary parameters are the number of values that must be specified to establish a numerical system. For example, for a ratio scale type zero means absence and one arbitrary parameter must be specified, such as an object of standard mass or length. For an interval scale type, zero is arbitrary and two parameters must be specified, such as the temperature at which water freezes and boils.  Meaningful comparisons indicates comparisons that can be made between measurements of each scale type.
