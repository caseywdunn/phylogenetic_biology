...
We can see now that each row has the exact same values. If you look closely, they should be familiar -- they are the equilibrium frequencies from $\mathbf{Pi}$. 
...
Let's start with the root of the tree. How do we decide on the state at the root? We could clamp it to a specific initial state, just as we did when looking at a single edge (Figure \@ref(fig:sim-jc-mu-sweep)). But let's pick it at random. We could pick the starting state from that bag with equal nucleotide frequencies, but some models don't have equal nucleotide frequencies. So let's pick the starting nucleotide from a bag with the frequencies in the $\Pi$ vector. 
